...
impl<'a> Camera<'a> {
    pub fn new<F: Facade>(facade: &F, width: f32, height: f32) -> Camera<'a> {
        ...
        Camera {
            projection: PerspectiveMatrix3::new(width / height, 60.0, 0.001, 10.0),
    ...
    pub fn draw(&self, facade: &GlutinFacade, particles: &Particles, time: f32) {
        let texture = &self.depth_steps.texture;
        let mut frame_texture = SimpleFrameBuffer::new(facade, texture).unwrap();

        let mut frame = (*facade).draw();
        frame.clear_color_srgb_and_depth(BACKGROUND, 1.0);
        frame_texture.clear_color_srgb(0.0, 0.0, 1.0, 0.0); // FIXME clear with another color

        let mut projection = self.projection;
        // println!("projection: {:?}", projection);
        projection.set_zfar(((time.cos() + 1.0) / 2.0) * 10.0);
